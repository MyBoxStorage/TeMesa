    ...
    <div className="h-full flex flex-col bg-background">
      ...
      <Tabs defaultValue="guest" className="flex-1 flex flex-col">
        ...
        <TabsContent value="guest" className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {/* Notes */}
          {(reservation.notes || reservation.dietaryNotes || reservation.occasion) && (
            <div className="space-y-2">
              {reservation.occasion && (
                <div className="flex items-start gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                  <Flag className="w-3.5 h-3.5 text-amber-400 mt-0.5 shrink-0" />
                  <p className="text-[12px] text-amber-300">{reservation.occasion}</p>
                </div>
              )}
              {reservation.dietaryNotes && (
                <div className="flex items-start gap-2 p-3 bg-muted/40 rounded-lg">
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-400 mt-0.5 shrink-0" />
                  <p className="text-[12px] text-muted-foreground">{reservation.dietaryNotes}</p>
                </div>
              )}
              {reservation.notes && (
                <div className="flex items-start gap-2 p-3 bg-muted/40 rounded-lg">
      ...
      <div className="px-4 py-3 border-t border-border bg-background">
        <div className="flex gap-2 flex-wrap">
          {reservation.status === 'CONFIRMED' && (
            <Button
              size="sm" className="flex-1 h-8 text-[12px] gap-1.5 bg-green-500 hover:bg-green-600 text-white"
          ...
          {reservation.status === 'CHECKED_IN' && (
            <Button
              size="sm" className="flex-1 h-8 text-[12px] gap-1.5"
              variant="secondary"
          ...
          {reservation.status === 'CONFIRMED' && (
            <Button
              size="sm" className="h-8 text-[12px] gap-1.5"
              variant="outline"
